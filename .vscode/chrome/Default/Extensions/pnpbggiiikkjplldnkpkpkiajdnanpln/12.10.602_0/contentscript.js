...
function evalOutFromSandbox(value)
{
	...
	var scriptText = "{ var r = document.getElementById('" + retValId + "'); if (r) { try { r.value = eval(\"" + value.replace(/[\\"]/g, "\\$&") + "\"); } catch(e) { r.value = JSON.stringify({\"error\": e.toString()}); }}}";
...
function injectIocmOnPage()
{
	...
	window[iocmName].nextCacheId = window[iocmName].OBJECTID_AGENT - 1;
	...
	window[iocmName].getObjectFromCache = function(objectId) {
		...
		if ((obj !== null) || (objectId < this.OBJECTID_AGENT))
			return obj;
	...
	window[iocmName].invoke = function(callData) {
		...
		try {
			...
			var marshalToPage = ((objectId == this.OBJECTID_WINDOW) && ((methodName == "Array") || (methodName == "eval"))) ||
				((flags == 2) && (methodName.indexOf("tcaInvokeObjectMethod") == 0));
			...
			if (typeof(params) == "object")
			{
				for (var i = 0; i < params.length; i++)
				{
					if ((typeof(params[i]) == "object") && (params[i] != null)
						&& (typeof(params[i]["objectId"]) == "number")
						&& (params[i]["objectId"] < this.OBJECTID_AGENT))
					{
...
{
	getIocm().port = null;
	getIocm().port = chrome.extension.connect({});
	...
	function (msg)
	{
		if (msg["setobject"])
		{
			...
			getIocm().context.deviceScaleFactor = msg["deviceScaleFactor"];
